...
void graphInit() {
    ...
    lastStart = BLOCK_LOW(group-1, group, nodeNumber);
    lastEnd = SIZE+1;
    /*
    int flag = 0;
    otherJob = ohter_R;
...
void SR() {
    ...
    for(rec = 0;rec < group;rec++) {
        ...
        */
        MPI_Send(&graphSize[pid_start*nodeNumber], (int)pid_size, MPI_UNSIGNED, (int)rec, (int)pid_start, MPI_COMM_WORLD);
    }
    unsigned start,count;
    for(start = 0,count = 0;start < SIZE && count < group;start+=pid_size) {
        if(start == (group - 1))
            MPI_Recv(&graphSize[lastStart*nodeNumber], (int)(lastEnd - lastStart), MPI_UNSIGNED, group-1, (int)start, MPI_COMM_WORLD, &status);
        else
            MPI_Recv(&graphSize[start*nodeNumber], (int)pid_size, MPI_UNSIGNED, start, (int)start, MPI_COMM_WORLD, &status);
        count++;
    }
    //(*otherJob)()
}
