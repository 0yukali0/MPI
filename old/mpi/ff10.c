...
void SR() {
    ...
    (*recv)(start, count);
    recv = Rusual;
    for(start = 0,count = 0;count < limit;start+=pid_size) {
        (*recv)(start, count);
        count++;
    }
    //(*otherJob)()
...
void Rlast(unsigned start, unsigned count) {
    MPI_Status status;
    MPI_Recv(&graphSize[start*nodeNumber], (int)(lastEnd - lastStart), MPI_UNSIGNED, count, (int)start, MPI_COMM_WORLD, &status);
}
void Rusual(unsigned start, unsigned count) {
    MPI_Status status;
