...
void SR() {
    ...
    (*recv)(start, count);
    recv = Rusual;
    for(start -= pid_size,count -= 1;count >= 0;start-=pid_size)
        (*recv)(start, count);
    //(*otherJob)()
}

void Rlast(unsigned start, unsigned count) {
    MPI_Recv(&graphSize[(group-1)*nodeNumber], (int)(lastEnd - lastStart), MPI_UNSIGNED, group-1, (int)start, MPI_COMM_WORLD, &status);
}
void Rusual(unsigned start, unsigned count) {
    MPI_Recv(&graphSize[start*nodeNumber], (int)pid_size, MPI_UNSIGNED, count, (int)start, MPI_COMM_WORLD, &status);
}